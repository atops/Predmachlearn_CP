...
Trying to be a little smarter about covariate selection, I made several exploratory plots, including box plots of each of the factors against the `classe` variable. Some of these are shown in the Appendix. These showed several outliers which were contained within only 3 observations. I threw those 3 observations out.
...
I then visually looked for a few variables that showed the highest relationship with classe from the box plots. I also used the `varImp` function from `caret` to identify which factors carried the most importance on the initial model. 
...
I took the x, y and z components of the most significant variables and added these to my model to see if I could a random forest model to complete. It took a while to complete but the results were very good.
...
tc <- trainControl(method="repeatedcv", 
                   ...
                   classProbs=TRUE,
                   allowParallel=TRUE)
trfit <- train(classe ~ roll_dumbbell + yaw_dumbbell +pitch_dumbbell +
...
Training model accuracy (in-sample) is as follows:
...
We expect the out-of-sample error to be greater than the in-sample error.


##D. Explanation of Choices

I chose the random forests method because it was taught in the lectures, is relatively easy to understand conceptually and because it is reported to produce good results. However, it took a long time to run.
I chose k-fold cross validation because it was straightforward to understand. 
I chose the factors to include in the model based on their relative importance based on an initial partial least squares model, and based on visual inspection of their variability compared with classe. I would have gone to more sophisticated lengths had the model not turned out well, or had it not taken so long to run with more variables.
...
```{r Boxplots, echo=FALSE, fig.height=15, fig.width=10, message=FALSE, warning=FALSE}
...
boxplots(trdf, "_z")
```
```{r Boxplot2, echo=FALSE, fig.width=10, message=FALSE, warning=FALSE}
